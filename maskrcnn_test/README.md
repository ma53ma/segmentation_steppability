# Customized changes
Updated to run the standard coco dataset
...
where XXX is the name of the test data folder; 0 refers to the order number of the experiments; depth_npy_0.npy is the point cloud file; color_image_0.png is the RGB gound truth image.
2. Modify the target test data folder in ```preprocess_kinect.py``` then run it. It will generate the processed depth image ```depth_img_0.png```
3. Modify the target test data folder in ```test.py``` then run it. It will generate the predicted mask label ```predict_mask_img_bin_XX_XX.png```, predicted mask area ```predict_mask_img_black_white_XX_XX.png``` and the visulized result ```visualized_image_XX.png```.
